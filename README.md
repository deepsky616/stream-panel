...
나이스는 `승인사항` 구역 제목을 누르지 않고 `미결/협조함`만 연 뒤 그 화면의 `Total` 숫자만 읽습니다. 에듀파인은 메뉴를 이동하지 않고 화면 상단 `결재(긴급)` 바로 옆 숫자만 읽습니다.
...
- 결재 대기 확인은 나이스 `미결/협조함` 화면의 `Total`, 에듀파인 상단 `결재(긴급)` 옆에 표시된 수량 하나만 앱으로 가져옵니다. `승인사항`과 `문서관리` 구역 제목은 클릭하지 않습니다.
...
- [ ] 나이스 업무 알림은 `승인사항`을 누르지 않고 `미결/협조함`을 연 뒤 `Total` 숫자만 읽습니다.
- [ ] 에듀파인 업무 알림은 메뉴를 누르지 않고 화면 상단 `결재(긴급)` 옆 숫자만 읽습니다.

...
그 밖의 나이스와 에듀파인 화면은 설정의 `내 웹 업무 만들기`에서 직접 키로 만들 수 있습니다. 업무 이름, 나이스 또는 에듀파인, 엣지 또는 크롬, 화면에서 누를 메뉴 이름과 마지막 도착 화면의 확인 문구를 입력합니다. 메뉴 이동은 최대 여덟 단계이며 주소, 선택자, 스크립트나 키 입력은 받지 않습니다. 사용자 지정 단계는 링크, 메뉴 또는 탭처럼 화면 이동 역할이 확인된 요소만 누릅니다.

전국 17개 시도 교육청을 지원합니다. 설정에서 소속 교육청을 바꾸면 기존 웹 업무 키의 주소도 함께 바뀌며 키 식별자, 선택한 업무용 브라우저와 멀티 액션 참조는 유지됩니다.
...
결재함 목록까지만 열리며 승인, 반려, 서명과 결재 처리는 화면에서 직접 해야 합니다.

스트림 패널은 시스템별 대기 수, 마지막 확인 시각과 마지막 알림 수만 저장합니다. 문서 제목,
작성자, 본문, 암호, 인증서, 쿠키와 로그인 자료는 읽어 저장하지 않습니다. 맥에서는 이 설정과
기능이 표시되지 않으며 결재 대기 확인 서비스도 시작하지 않습니다.
...
- 결재 대기 확인은 허용된 결재함과 대기 문서 메뉴만 이동하며 수량 하나만 앱으로 가져옵니다.
- 결재 대기 상태 파일에는 수량과 시각만 남고 문서 제목, 작성자, 본문과 인증 자료는 남지 않습니다.
...
- [ ] 결재함 목록까지만 열고 승인, 반려, 서명과 결재 처리 단추를 누르지 않습니다.
- [ ] 결재 대기 상태 파일에는 수량과 시각만 있고 문서 제목, 작성자, 본문, 쿠키와 인증 자료가 없습니다.

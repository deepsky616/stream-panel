...
나이스와 K-에듀파인 업무 키는 업무포털 메뉴를 자동 클릭하지 않습니다. 복무·출장·기안·품의 키를 누를 때마다 해당 시스템의 기존 신청·작성 팝업을 안전하게 닫고 오른쪽 새 탭에서 처음부터 실행합니다. 업무 알림 확인은 기존 작업 탭을 바꾸지 않고 오른쪽 임시 탭에서 수행하며, 성공하면 임시 탭을 자동으로 닫고 로그인·인증·탐색 오류가 있을 때만 해당 탭을 남겨 앞으로 표시합니다. 나이스와 K-에듀파인은 서로 다른 탭과 로그인 상태로 관리하며 업무포털 탭은 덮어쓰지 않습니다. 최소화된 업무용 브라우저는 실행 전에 복원하고, 로그인 화면이 나타나면 사용자가 해당 시스템 탭에서 로그인한 뒤 같은 실행이 자동으로 이어지므로 키를 다시 누를 필요가 없습니다. 같은 키를 연속으로 눌러도 작업은 하나만 실행되고 현재 업무용 브라우저만 앞으로 가져옵니다.
...
나이스는 `승인사항` 구역 제목을 누르지 않고 `미결/협조함`을 연 뒤 목록의 전체 건수를 읽습니다. 에듀파인은 `문서관리` 구역 제목을 누르지 않고 `결재`, `결재대기` 순서로 이동한 뒤 목록의 전체 건수를 읽습니다. 목록에 `Total` 또는 `총`이 있으면 그 값을 우선 사용하고, 없을 때만 결재 목록 헤더가 있는 실제 표·그리드의 데이터 행 수를 사용합니다. 페이지 크기 선택에 쓰이는 `전체 100` 같은 값은 건수로 사용하지 않습니다. 제목 표시줄에는 자물쇠 왼쪽에 에듀파인, 나이스 순서로 항상 표시됩니다.

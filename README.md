...
나이스와 K-에듀파인 업무 키 자체는 업무포털 메뉴를 자동 클릭하지 않습니다. 설정에서 `업무포털 열기`를 누르고 메인 로그인을 완료한 뒤 `나이스·에듀파인 연결`을 누르면 공식 연결 항목을 나이스, K-에듀파인 순서로 실행합니다. 연결 항목은 같은 출처 iframe뿐 아니라 다른 출처의 포털 프레임에서도 안전하게 탐색합니다. 복무·출장·기안·품의 키를 누를 때는 시스템별 로그인 기준 탭 하나를 유지해 세션을 보존하고 이전 신청·작성 탭을 정리한 뒤 같은 업무용 브라우저 창의 새 작업 탭에서 처음부터 실행합니다. 업무 알림 확인은 기존 작업 탭을 바꾸지 않고 임시 탭에서 수행하며, 성공하면 임시 탭을 자동으로 닫고 로그인·인증·탐색 오류가 있을 때만 해당 탭을 남겨 앞으로 표시합니다. 최소화된 업무용 브라우저는 실행 전에 복원하며 같은 키를 연속으로 눌러도 작업은 하나만 실행됩니다.

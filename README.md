...
2. `확장 기능 폴더 열기`를 누릅니다.
3. 사용할 엣지 또는 크롬의 `확장 주소 복사`를 누릅니다.
4. 해당 브라우저 주소창에 붙여넣고 엔터 키를 누른 뒤 개발자 모드를 켭니다.
   엣지는 `edge://extensions/`, 크롬은 `chrome://extensions/`입니다.
5. `압축 해제된 확장 기능을 불러오기`를 누릅니다.
6. 앞에서 연 `browser-extension` 폴더를 선택합니다.
7. 스트림 패널로 돌아가 해당 브라우저의 `연결 시험`을 누릅니다.
8. 오른쪽 액션 목록의 `웹 업무`에서 원하는 키를 패널에 놓습니다.

브라우저 보안 화면은 다른 앱이 일반 웹 주소처럼 직접 열 수 없습니다. 그래서 `확장 주소 복사`는 빈 새 창을 만들지 않고 정확한 내부 주소를 클립보드에 복사합니다.

크롬 웹 스토어와 엣지 추가 기능 등록 전까지는 위와 같이 포함된 폴더를 직접 불러옵니다. 정식 등록 뒤에는 설정의 설치 단추만 스토어 주소로 바꾸고 연결 구조는 그대로 사용합니다.

...
나이스 업무 키는 기존 나이스 탭, 최초 빈 탭, 업무포털 탭 순서로 재사용합니다. 업무포털에서 나이스로 들어가야 할 때는 선택한 교육청의 승인된 나이스 링크만 사용하고, 최소화된 업무용 브라우저를 먼저 복원합니다. 로그인 화면이 나타나면 사용자가 로그인한 뒤 같은 실행이 자동으로 이어지므로 키를 다시 누를 필요가 없습니다. 같은 키를 연속으로 눌러도 작업은 하나만 실행되고 현재 업무용 브라우저만 앞으로 가져옵니다.

처음 사용할 때 다음 순서로 연결합니다.

1. 편집기에서 `설정`, `웹 업무 연결`을 엽니다.
2. 소속 교육청과 업무용 엣지 또는 크롬을 고릅니다. 엣지를 권장합니다.
3. `업무용 브라우저 열기`를 누르고 나이스와 에듀파인에 직접 로그인합니다.
4. `연결 시험`으로 교육청 포털이 열리고 상태가 `준비됨`으로 바뀌는지 확인합니다.
5. 기본 업무는 오른쪽 액션 목록의 `웹 업무`에서 원하는 키를 패널에 놓습니다.
...
5. 업무용 브라우저에 직접 로그인한 뒤 `지금 확인`을 눌러 대기 수를 확인합니다.

...
메뉴와 화면 표식은 접근 가능한 `iframe`과 `frame`을 안쪽까지 재귀 탐색하고, 실제로 표시되고 활성화된 요소만 정확한 이름으로 선택합니다. 업무용 브라우저가 열려 있는 동안 30초마다 나이스·에듀파인 탭을 확인하고, `btnUseTimeExtn` 세션 연장 팝업이 나타나면 표시된 버튼의 이름이 허용 목록과 정확히 일치하고 후보가 하나일 때만 자동으로 연장합니다.
...
나이스는 `승인사항` 구역 제목을 누르지 않고 `미결/협조함`을 연 뒤 실제 목록 행을 셉니다. 에듀파인은 `문서관리` 구역 제목을 누르지 않고 `결재`, `결재대기` 순서로 이동한 뒤 실제 목록 행을 셉니다. 열린 결재 목록 표·그리드가 비어 있으면 두 시스템 모두 `0건`으로 처리합니다. 목록 표를 확인할 수 있을 때는 그 행 수를 가장 먼저 사용하고, 목록 구조를 확인할 수 없을 때만 `Total` 또는 `총` 값을 보조로 사용하므로 페이지 크기의 `전체 100`이나 오래된 요약 숫자가 건수로 표시되지 않습니다. 제목 표시줄에는 자물쇠 왼쪽에 에듀파인, 나이스 순서로 항상 표시됩니다.

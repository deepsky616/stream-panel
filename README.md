...
파워 오토메이트 없이 엣지나 크롬에서 정해진 업무 화면까지 이동할 수 있습니다. 새 브라우저를 설치하는 방식이 아니라 스트림 패널에 포함된 작은 브라우저 확장 기능을 한 번 추가하는 방식입니다.
...
웹 업무 키에는 실제로 확장 기능을 설치한 브라우저와 프로필을 선택해야 합니다. 사이트에 로그인하지 않은 상태라면 사용자가 로그인한 뒤 키를 다시 누릅니다.

...
그 밖의 나이스와 에듀파인 화면은 설정의 `내 웹 업무 만들기`에서 직접 키로 만들 수 있습니다. 업무 이름, 교육청, 나이스 또는 에듀파인, 엣지 또는 크롬, 화면에서 누를 메뉴 이름과 마지막 도착 화면의 확인 문구를 입력합니다. 만든 키를 편집기에서 다시 선택하면 같은 화면에서 교육청과 이동 경로를 고쳐 저장할 수 있습니다. 메뉴 이동은 최대 여덟 단계이며 주소, 선택자, 스크립트나 키 입력은 받지 않습니다. 사용자 지정 단계는 링크, 메뉴 또는 탭처럼 화면 이동 역할이 확인된 요소만 누릅니다.
...
나이스와 K-에듀파인 업무 키는 기존 업무 시스템 탭, 최초 빈 탭, 업무포털 탭 순서로 재사용합니다. 업무포털에서 업무 시스템으로 들어갈 때는 선택한 교육청의 승인된 SSO 메뉴만 사용하고, 최소화된 업무용 브라우저를 먼저 복원합니다. 로그인 화면이 나타나면 사용자가 로그인한 뒤 같은 실행이 자동으로 이어지므로 키를 다시 누를 필요가 없습니다. 같은 키를 연속으로 눌러도 작업은 하나만 실행되고 현재 업무용 브라우저만 앞으로 가져옵니다.
...
3. `업무포털 로그인 후 자동 연결`을 켜고 `나이스`, `K-에듀파인` 또는 기본값인 `둘 다`를 고릅니다.
4. `업무용 브라우저 열기`를 누른 뒤 업무포털에서 한 번 로그인합니다.
5. 포털 로그인이 끝나면 공식 SSO 메뉴를 통해 선택한 시스템이 차례로 준비됩니다. 카드에서 시스템별 `연결 중`, `연결됨`, `추가 로그인 필요`, `실패` 상태를 확인할 수 있습니다.
6. 기본 업무는 오른쪽 액션 목록의 `웹 업무`에서 원하는 키를 패널에 놓습니다.
7. 다른 화면은 같은 설정 탭의 `내 웹 업무 만들기`에서 메뉴 이름을 보이는 순서대로 적고 `키로 추가`를 누릅니다.

자동 연결 과정에서 만들어진 임시 탭은 인증 확인 뒤 정리하며, 최초 빈 탭은 업무포털 탭으로 재사용합니다. 나이스나 K-에듀파인 서버가 실제 세션을 만료시킨 경우에는 해당 시스템이 요구하는 추가 인증을 사용자가 직접 완료해야 합니다.
...
5. 업무포털 자동 연결 상태를 확인한 뒤 `지금 확인`을 눌러 준비된 나이스 또는 K-에듀파인 세션으로 대기 수를 확인합니다.
...
- 작성 화면을 연 뒤 멈추며 저장, 제출, 상신, 승인, 결재, 확정, 삭제, 취소, 확인, 지급, 송금, 이체, 발송, 등록, 신청, 요청, 완료, 반려, 서명, 동의, 전송, 처리와 인증 입력 단추를 사용자 지정 단계에서 누르지 않습니다.
...
- 결재 대기 확인은 나이스의 결재함, 에듀파인의 업무관리와 결재함 메뉴까지만 이동하며 화면에 표시된 수량 하나만 앱으로 가져옵니다.
...
- [ ] 저장, 제출, 결재, 삭제, 확인이나 인증 입력 문구를 단계에 넣으면 키가 만들어지지 않습니다.

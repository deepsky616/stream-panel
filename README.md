...
전국 17개 시도 교육청을 지원합니다. 기본 웹 업무 키를 패널에 놓은 뒤 속성에서 키마다 교육청을 선택할 수 있습니다. 설정의 소속 교육청은 새 키의 기본값으로 쓰며, 이전 버전에서 만든 교육청 정보 없는 키는 처음 한 번 새 형식으로 옮깁니다. 이미 교육청을 저장한 키와 멀티 액션 참조는 그대로 유지됩니다.

나이스와 K-에듀파인 업무 키는 업무포털 메뉴를 자동 클릭하지 않습니다. 기존의 해당 시스템 탭을 먼저 재사용하고, 없으면 최초 빈 탭, 그마저 없으면 교육청별 시스템 전용 새 탭을 사용합니다. 나이스와 K-에듀파인은 서로 다른 탭과 로그인 상태로 관리하며 업무포털 탭은 덮어쓰지 않습니다. 최소화된 업무용 브라우저는 실행 전에 복원하고, 로그인 화면이 나타나면 사용자가 해당 시스템 탭에서 로그인한 뒤 같은 실행이 자동으로 이어지므로 키를 다시 누를 필요가 없습니다. 같은 키를 연속으로 눌러도 작업은 하나만 실행되고 현재 업무용 브라우저만 앞으로 가져옵니다.

메뉴와 화면 표식은 접근 가능한 `iframe`과 `frame`을 안쪽까지 재귀 탐색하고, 실제로 표시되고 활성화된 요소만 정확한 이름으로 선택합니다. 작업 도중 `btnUseTimeExtn` 세션 연장 팝업이 나타나면 표시된 버튼의 이름이 허용 목록과 정확히 일치하고 후보가 하나일 때만 자동으로 연장합니다.

기본 웹 업무의 고정 이동 경로는 다음과 같습니다.

- 에듀파인 기안: `업무관리 → 문서관리 → 기안 → 공용서식 → 표준서식(결재4인,협조4인)`
- 에듀파인 품의: `학교회계 → 사업담당 → 품의/정산 → 품의등록`

기안은 정확한 표준서식만 선택하며, WXSClient가 이미 실행 중인 단일 창을 재사용하는 경우도 감지해 앞으로 가져옵니다. 품의는 등록 화면의 제목·개요·예산·품목 같은 안정적인 입력 영역을 확인한 뒤 이동 완료로 처리합니다.

처음 사용할 때 다음 순서로 연결합니다.

1. 편집기에서 `설정`, `웹 업무 연결`을 엽니다.
2. 소속 교육청과 업무용 엣지 또는 크롬을 고릅니다. 엣지를 권장합니다.
3. `나이스·K-에듀파인 직접 연결`을 켜고 `나이스`, `K-에듀파인` 또는 기본값인 `둘 다`를 고릅니다.
4. `선택 시스템 열기`를 누르면 선택한 시스템이 각각의 전용 탭에서 열립니다.
5. 추가 로그인이 필요한 시스템만 해당 탭에서 직접 로그인합니다. 카드에서 시스템별 `연결 중`, `연결됨`, `추가 로그인 필요`, `실패` 상태를 확인할 수 있습니다.
6. 기본 업무는 오른쪽 액션 목록의 `웹 업무`에서 원하는 키를 패널에 놓습니다.
7. 다른 화면은 같은 설정 탭의 `내 웹 업무 만들기`에서 메뉴 이름을 보이는 순서대로 적고 `키로 추가`를 누릅니다.

직접 연결은 최초 빈 탭을 첫 시스템 탭으로 재사용하고, 두 시스템을 선택하면 나머지 시스템만 별도 탭으로 엽니다. 나이스나 K-에듀파인 서버가 실제 세션을 만료시켜 로그인 화면으로 돌아간 경우에는 해당 시스템이 요구하는 추가 인증을 사용자가 직접 완료해야 합니다.

### 결재 대기 업무 알림

윈도우에서는 나이스와 에듀파인의 결재 대기 건수를 일정한 간격으로 확인해 패널 키 배지와 윈도우
알림으로 받을 수 있습니다. 처음에는 꺼져 있으므로 필요한 시스템만 직접 켭니다.
...
5. 시스템별 직접 연결 상태를 확인한 뒤 `지금 확인`을 눌러 준비된 나이스 또는 K-에듀파인 세션으로 대기 수를 확인합니다.
6. `결재함 키 추가`를 누르면 맨 앞 화면의 첫 빈 위치에 키가 생깁니다.

나이스는 `승인사항` 구역 제목을 누르지 않고 그 안의 `미결/협조함`만 엽니다. 에듀파인은 `결재(긴급)`을 먼저 찾고, 없으면 `문서관리` 구역 제목을 누르지 않은 채 그 구역의 `결재 → 결재대기`를 엽니다. 선택된 탭 또는 목록이 준비된 뒤 표시된 배지나 전체 건수를 읽습니다.
...
- 기본 업무는 작성 화면을 연 뒤 멈춥니다. 사용자 지정 업무의 저장, 제출, 결재, 등록, 신청, 확인, 인증 입력은 매 실행 직전 경고 창에서 명시적으로 확인한 경우에만 한 번 누릅니다. 삭제, 상신, 승인, 확정, 취소, 지급, 송금, 이체, 발송, 요청, 완료, 반려, 서명, 동의, 전송과 처리는 계속 차단합니다.
...
- 결재 대기 확인은 나이스 `승인사항` 구역의 `미결/협조함`, 에듀파인 `결재(긴급)` 또는 `문서관리` 구역의 `결재 → 결재대기`까지만 이동하며 화면에 표시된 수량 하나만 앱으로 가져옵니다. `승인사항`과 `문서관리` 구역 제목은 클릭하지 않습니다.
...
- [ ] 소속 교육청을 바꾸면 기존 웹 업무 키 주소만 바뀌고 키 식별자와 멀티 액션 참조는 유지됩니다.
...
- [ ] 업무 알림은 처음에 꺼져 있고 나이스와 에듀파인을 각각 켤 수 있습니다.
- [ ] `지금 확인` 뒤 해당 시스템의 결재 대기 수와 상태가 표시됩니다.
- [ ] 나이스 업무 알림은 `승인사항`을 누르지 않고 `미결/협조함`만 엽니다.
- [ ] 에듀파인 업무 알림은 `결재(긴급)` 또는 `문서관리` 구역의 `결재 → 결재대기`를 열며 `문서관리` 제목은 누르지 않습니다.
- [ ] `결재함 키 추가` 뒤 맨 앞 화면의 첫 빈 위치에 키가 생기고 대기 수 배지가 보입니다.

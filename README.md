...
나이스와 K-에듀파인 업무 키는 업무포털 메뉴를 자동 클릭하지 않습니다. 복무·출장·기안·품의 키를 누를 때마다 해당 시스템의 기존 업무 탭과 신청·작성 팝업을 안전하게 닫고 전용 탭에서 처음부터 새로 실행합니다. 업무 알림 확인과 연결 확인은 기존 로그인 탭을 재사용합니다. 나이스와 K-에듀파인은 서로 다른 탭과 로그인 상태로 관리하며 업무포털 탭은 덮어쓰지 않습니다. 최소화된 업무용 브라우저는 실행 전에 복원하고, 로그인 화면이 나타나면 사용자가 해당 시스템 탭에서 로그인한 뒤 같은 실행이 자동으로 이어지므로 키를 다시 누를 필요가 없습니다. 같은 키를 연속으로 눌러도 작업은 하나만 실행되고 현재 업무용 브라우저만 앞으로 가져옵니다.

...
윈도우에서는 파워 오토메이트 없이 엣지나 크롬에서 정해진 업무 화면까지 이동할 수 있습니다. 새 브라우저를 설치하는 방식이 아니라 윈도우용 스트림 패널에 포함된 작은 브라우저 확장 기능을 한 번 추가하는 방식입니다.

맥에서는 나이스·에듀파인 웹 업무 연결을 지원하지 않습니다. 웹 업무 액션과 연결 설정이 표시되지 않고, 로컬 연결부도 시작하지 않으며, 맥 앱 묶음에는 확장 기능이 들어가지 않습니다. 윈도우에서 만든 기존 웹 업무 키를 맥에서 실행하면 자동 이동 없이 사이트만 엽니다.

현재 제공하는 웹 업무 키는 다음 네 가지입니다.
...
공식 스토어 등록이 끝난 뒤에는 처음 한 번 다음 순서로 연결합니다.

1. 편집기에서 `설정`, `웹 업무 연결`을 엽니다.
2. 사용할 엣지 또는 크롬의 `확장 기능 설치`를 누릅니다.
3. 열린 공식 스토어에서 확장 기능 추가를 승인합니다.
4. 스트림 패널로 돌아가 해당 브라우저의 `연결 시험`을 누릅니다.
5. 오른쪽 액션 목록의 `웹 업무`에서 원하는 키를 패널에 놓습니다.

공식 스토어 심사 전 개발 빌드는 다음 임시 순서를 사용합니다.

1. `확장 기능 폴더 열기`를 누릅니다.
2. 사용할 브라우저의 `확장 주소 복사`를 누릅니다.
3. 해당 브라우저 주소창에 붙여넣고 엔터 키를 누른 뒤 개발자 모드를 켭니다.
   엣지는 `edge://extensions/`, 크롬은 `chrome://extensions/`입니다.
4. `압축 해제된 확장 기능을 불러오기`를 누르고 앞에서 연 `browser-extension` 폴더를 선택합니다.
5. 스트림 패널로 돌아가 해당 브라우저의 `연결 시험`을 누릅니다.

브라우저 보안 화면은 다른 앱이 일반 웹 주소처럼 직접 열 수 없습니다. 그래서 `확장 주소 복사`는 빈 새 창을 만들지 않고 정확한 내부 주소를 클립보드에 복사합니다.

스토어 제출용 묶음은 `npm run extension:package`로 만듭니다. 결과는 `dist/StreamPanel-Web-Connector-1.0.0.zip`이며, 제출 문구와 권한 설명은 `docs/browser-extension-store.md`, 개인정보 처리 안내는 `PRIVACY.md`에 있습니다.

윈도우의 웹 업무 키에는 실제로 확장 기능을 설치한 브라우저와 프로필을 선택해야 합니다. 사이트에 로그인하지 않은 상태라면 사용자가 로그인한 뒤 키를 다시 누릅니다.

보안 범위는 코드로 고정되어 있습니다.

- 경기 나이스와 경기 에듀파인 주소에서만 작동합니다.
- 암호, 인증서와 로그인 자료를 읽거나 저장하지 않습니다.
- 미리 정한 네 가지 명령 외의 임의 명령과 스크립트는 받지 않습니다.
- 작성 화면을 연 뒤 멈추며 저장, 제출, 상신, 승인과 결재 단추를 누르지 않습니다.
- 스트림 패널과 확장 기능은 컴퓨터 안의 `127.0.0.1` 주소로만 통신합니다.
- 자동 이동에 실패해도 사이트는 열리며 실패 지점과 다시 연결하는 방법을 알립니다.
...
- [ ] 엣지와 크롬의 확장 기능을 각각 불러올 수 있습니다.
- [ ] 연결 시험을 마치면 설정에 `지금 연결됨`이 표시됩니다.
- [ ] 나이스 복무 키가 선택한 브라우저와 프로필로 나이스를 엽니다.
...
- [ ] 확장 기능을 끄면 사이트만 열리고 다시 연결하는 방법이 표시됩니다.
...
윈도우 설치 파일은 윈도우에서, 맥 디스크 이미지는 맥에서 만들어야 합니다. 깃허브 자동 작업이 두 운영체제 빌드를 대신 수행합니다. 리눅스는 지원 대상이 아닙니다. 파비콘과 새 버전 확인을 제외하면 실행 중 외부 네트워크를 사용하지 않습니다. 웹 업무 연결은 외부 서버가 아니라 컴퓨터 안의 `127.0.0.1`만 사용합니다.

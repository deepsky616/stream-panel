...
별도의 사전 연결 설정은 필요하지 않습니다. 각 업무 키와 업무 알림의 `지금 확인`이 기존 로그인 탭을 우선 재사용하고, 탭이 없을 때만 최초 빈 탭이나 시스템 전용 새 탭을 사용합니다. 나이스나 K-에듀파인 서버가 실제 세션을 만료시켜 로그인 화면으로 돌아간 경우에는 해당 시스템이 요구하는 추가 인증을 사용자가 직접 완료해야 합니다.
...
5. `지금 확인`을 누르면 기존 로그인 탭을 재사용해 나이스 또는 K-에듀파인 대기 수를 확인합니다.

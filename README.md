...
나이스는 `승인사항` 구역 제목을 누르지 않고 `미결/협조함`을 연 뒤 목록의 전체 건수를 읽습니다. 에듀파인은 `문서관리` 구역 제목을 누르지 않고 `결재`, `결재대기` 순서로 이동한 뒤 목록의 전체 건수를 읽습니다. 목록에 `Total`, `총`, `전체`가 있으면 그 값을 우선 사용하고, 없을 때만 실제 표·그리드의 데이터 행 수를 사용합니다. 제목 표시줄에는 자물쇠 왼쪽에 에듀파인, 나이스 순서로 항상 표시됩니다.
